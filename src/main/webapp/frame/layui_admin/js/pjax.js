	...
	! function(t) {
		...
		function o(e) {
			...
			e.beforeSend = function(t, a) {
				...
				var l = o.state,
					...
					m = y(a, u, e),
					v = h(m.url);
				if(r && (v.hash = r, m.url = v.href), f && d && f !== d) s(m.url);
				else if(m.contents) {
					o.state = {
						id: e.id || p(),
						url: m.url,
					...
					var g = e.scrollTo;
					if(r) {
						var j = decodeURIComponent(r.slice(1)),
							b = document.getElementById(j) || document.getElementsByName(j)[0];
						b && (g = t(b).offset().top)
					}
		...
		function v(e, n) {
			return e && n ? n.container = e : n = t.isPlainObject(e) ? e : {
				container: e
			}, n.container && (n.container = x(n.container)), n
		}
		...
		function y(e, n, a) {
			...
			if(r.url = i ? d(h(i)) : a.requestUrl, o) var s = t(j(e.match(/<head[^>]*>([\s\S.]*)<\/head>/i)[0])),
				c = t(j(e.match(/<body[^>]*>([\s\S.]*)<\/body>/i)[0]));
			else s = c = t(j(e));
			if(0 === c.length) return r;
			if(r.title = g(s, "title").last().text(), a.fragment) {
				if("body" === a.fragment) u = c;
				else var u = g(c, a.fragment).first();
